...
#' leaf_ids experienced by unit `i`.
...
#' be equal to the unique elements of leaf_ids. The vertices of `mytree`, `V(mytree)`, may have
#' an attribute `levels` containing integer values from 1 to `max(V(mytree)$levels)`.
#' In this case, the levels attribute specifies groups of nodes that share common
#' hyperparameters `rho[f]`, `tau_1[f]`, and `tau_2[f]`. If `V(mytree)$levels` is `NULL`,
...
#' the same node; same for tau_2 (gamma's). Default is `FALSE`
...
#' It is recommended to choose `nrestarts > 1`.
#' The default is 3.
#' @param keep_restarts If `TRUE`, the results from all random restarts will be returned.
#' If `FALSE`, only the restart with the highest objective function is returned. '
#' Default is `TRUE`.
...
#' logged to a text file in `log_dir`. If `FALSE` and `nrestarts > 1`,
...
#' for each individual leaf_ids (see [initialize_tree_lcm()]).
