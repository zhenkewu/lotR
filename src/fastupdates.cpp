...
//         //pre_resA(0) = resA(j,k);
//         //pre_resA(1) = log(2.0) + log(std::min(std::max(rmat(ii,k),1.0e-200),1.0-1e-200)) + log(g_psi(vv,j,k));
//         resA(j,k) += log(2.0) + log(std::min(std::max(rmat(ii,k),1.0e-200),1.0-1e-200)) + log(g_psi(vv,j,k));
//         // resA(j,k) += 2.0*rmat(ii,k)*g_psi(vv,j,k);
//         resB(j,k) += rmat(ii,k)*(X(ii,j)*0.5- 2.0*g_psi(vv,j,k)*(E_beta(j,k,vv)-E_zeta_u(j,k)));
//       }
//       logresBsq_o_A(j,k) = 2.0*log(abs(resB(j,k)))-resA(j,k);
...
//       vv = (int) v_lookup(ii)-1;
//
//       resD(k) += 0.5*rmat(ii,k);
//       for (int m=k;m<K;m++){
//         resC(k) += log(2.0)+log(rmat(ii,m))+log(g_phi(vv,k));
//         resD(k) += -0.5*rmat(ii,m)-2.0*rmat(ii,m)*g_phi(vv,k)*(E_eta(vv,k)-E_xi_u(k));
//       }
//       resD(k) += 0.5*rmat(ii,k);
//     }
//     logresDsq_o_C(k) = 2.0*log(abs(resD(k)))-resC(k);
...
//   for (int k=0;k<K;k++){
//     for (int j=0;j<J;j++){
//       resA(j,k) = 1.0/tau_2_t_u(j,k);//-log(h_pau(uu));
//       for (int i=0;i<n;i++){
//         ii = (int) subject_ids(i)-1;
//         vv = (int) v_lookup(ii)-1;
//         // update gamma:
...
//
//   for (int k=0;k<K-1;k++){
//     resC(k) = 1.0/tau_1_t_u(k)/h_pau(uu);
//       for (int i=0;i<n;i++){
//         ii = (int) subject_ids(i)-1;
//         vv = (int) v_lookup(ii)-1;
//
...
//   }
//
//   logresBsq_o_A = 2.0*log(abs(resB))-log(resA);
//   resA = 1.0/resA;
//
//   logresDsq_o_C = 2.0*log(abs(resD))-log(resC);
//   resC = 1.0/resC;
//   return List::create(Named("resA")=resA,
//                       Named("resB")=resB,
//                       Named("logresBsq_o_A")=logresBsq_o_A,
